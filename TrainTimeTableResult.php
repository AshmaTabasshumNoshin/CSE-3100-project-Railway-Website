...
<?php
    ...
    if(strlen($query1) >= $min_length){ // if query length is more or equal minimum length then
        ...
        if(mysqli_num_rows($raw_results) > 0){ // if one or more rows are returned do following
            ...
            echo "<div class='container' id='searchResult'><h3> From:".$results['sfrom'].'<br> <h3>To:'.$results['sto'].'</h3><br>Starting Time:'.$results['StartingTime'].'<br>Arriving Time:'.$results['ReachedTime'].'<br></div>';
